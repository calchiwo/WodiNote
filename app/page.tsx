  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        if (!isInstalled) {
          const promptShown = localStorage.getItem("wodinote_install_prompt_shown")
          if (!promptShown) {
  ...
  const handleSaveNote = async (title: string, content: string) => {
    ...
    try {
      ...
      if (editingNote) {
        ...
        if (decryptedNotes.length === 0 && !hasShownInstallPrompt) {
          const isInstalled = window.matchMedia("(display-mode: standalone)").matches
          if (!isInstalled) {
            setTimeout(() => {
              setShowInstallPrompt(true)
              setHasShownInstallPrompt(true)
              localStorage.setItem("wodinote_install_prompt_shown", "true")
            }, 2000)
          }
        }
      }
